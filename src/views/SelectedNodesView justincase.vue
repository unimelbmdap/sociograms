...
const buildAdjacencyList = (theseEdges) => {

  if (!theseEdges || typeof theseEdges !== "object" || Object.keys(theseEdges).length === 0) {
    console.warn("No edges available to build adjacency list.");
    return adjacencyList;
  }

 
  for (const key in theseEdges) {
    const edge = theseEdges[key];
    if (edge && edge.source && edge.target) {
      ...
      console.warn("Edge is missing source or target:", edge);
    }
  }
  return adjacencyList;
};
...
const findChildren = (node) => {
  ...
  const traverse = (currentNode) => {
    if (visited.has(currentNode)) return; // Prevent infinite recursion
    visited.add(currentNode);

    const directChildren = adjacencyList.value[currentNode] || [];
...
const connectedNodes = computed(() => {
  ...
  return findChildren(selectedNode.value); // Return directly, avoid side effects
});
